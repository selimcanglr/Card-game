...
public class MyCardGame {
    public static void main(String[] args) {
        ...
        do {
            // display menu
            System.out.println();
            System.out.println( "MyCardGame   Round: " + ( game.getRoundNo() + 1 ) + "\t TurnOfPlayer: "
                    + ( game.getTurnOfPlayerNo() + 1 ));
            System.out.println();
        ...
        } while ( selection != MENU_EXIT && game.getRoundNo() != 13);
    ...
    private static boolean play( Player p, CardGame game) {
        ...
        // Get the card an see if it is accepted
        c = p.playCard();
        accepted = game.playTurn( p, c);

        // If not accepted give the card back
        if ( ! accepted) {
            p.add( c);
        }
        else {
